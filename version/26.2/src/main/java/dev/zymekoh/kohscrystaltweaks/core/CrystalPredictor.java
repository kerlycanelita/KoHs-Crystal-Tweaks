...
public final class CrystalPredictor {
    ...
    public static EndCrystal findRealCrystalForLocal(Entity entity) {
    ...
    public static boolean queueLocalCrystalAttack(Entity entity) {
        ...
        long positionKey = LOCAL_ENTITY_TO_POSITION.get(entity.getId());
        LocalPrediction prediction = positionKey == NO_POSITION ? null : PREDICTIONS.get(positionKey);
        if (prediction == null || prediction.entity != entity || prediction.entity.isRemoved()) {
            return false;
        }

        // One boolean deliberately deduplicates butterfly clicks. No entity ID is
        // guessed and no packet is sent until the real server crystal is loaded.
        prediction.pendingAttack = true;
        return true;
    }
    ...
    public static void onSuccessfulCrystalUse(BlockHitResult hit) {
        ...
        if (existing != null) {
            if (existing.pendingAttack || existing.pairedRealEntityId != -1) {
    ...
    public static void onEntityLoaded(Entity entity) {
        ...
        if (consumePendingAttack(positionKey, prediction, realCrystal)) {
            return;
        }
    ...
    private static boolean consumePendingAttack(
            long positionKey,
            LocalPrediction prediction,
            EndCrystal realCrystal) {
        if (!prediction.pendingAttack) {
            return false;
        }
    ...
    private static void targetLocalCrystal(EndCrystal crystal) {
        if (crystal == null || crystal.isRemoved()) {
            return;
        }
        MINECRAFT.crosshairPickEntity = crystal;
        MINECRAFT.hitResult = new EntityHitResult(crystal);
    }
