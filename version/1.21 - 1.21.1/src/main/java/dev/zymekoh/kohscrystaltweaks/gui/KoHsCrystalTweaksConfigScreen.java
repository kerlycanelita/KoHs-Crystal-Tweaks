...
public final class KoHsCrystalTweaksConfigScreen extends Screen {
    ...
    public void renderBackground(DrawContext ctx, int mx, int my, float delta) {
        ...
        ctx.drawBorder(pxl, p, PANEL_W, PANEL_H, 0xE0B86BFF);

        // Content-area border
        int cy = py() + CONTENT_TOP - 4;
        ctx.drawBorder(pxl + 4, cy, PANEL_W - 8, PANEL_H - CONTENT_TOP - 22, 0x60B86BFF);
    }
    ...
    private void renderPicker(DrawContext ctx, int x, int y) {
        ...
        ctx.drawBorder(x - 1, y - 1, PICKER_W + 2, PICKER_H + 2, 0xE0B86BFF);

        int mX = x + Math.round(cs.saturation * (PICKER_W - 1));
        int mY = y + Math.round((1f - cs.value) * (PICKER_H - 1));
        ctx.drawBorder(mX - 3, mY - 3, 7, 7, 0xFFFFFFFF);
        ctx.drawBorder(mX - 2, mY - 2, 5, 5, 0xFF201826);

        int hx = x + PICKER_W + HUE_GAP;
        ...
        ctx.drawBorder(hx - 1, y - 1, HUE_BAR_W + 2, PICKER_H + 2, 0xE0B86BFF);

        int hm = y + Math.round(cs.hue * (PICKER_H - 1));
        ctx.drawBorder(hx - 2, hm - 2, HUE_BAR_W + 4, 5, 0xFFFFFFFF);
    }
    ...
    private void drawSwatch(DrawContext ctx, Text label, ColorState cs, boolean active, int x, int y) {
        ...
        ctx.drawBorder(x, y, SWATCH_W, SWATCH_H, active ? 0xFFFFE38D : 0xE0B86BFF);
        ctx.fill(x + 3, y + 3, x + 19, y + 19, cs.toArgb());
        ctx.drawBorder(x + 3, y + 3, 16, 16, 0xFFFFFFFF);
