...
public final class KoHsCrystalTweaksConfigScreen extends Screen {
    ...
    private int drawWrapped(DrawContext ctx, Text text, int cx, int y, int maxW, int color) {
        ...
        for (OrderedText line : lines) {
            drawOrderedText(ctx, line, cx - textRenderer.getWidth(line) / 2, ly, color);
            ly += 10;
        }
        return ly;
    }
    ...
    private void drawDesc(DrawContext ctx, String text, int x, int y) {
        drawOrderedText(ctx, Text.literal(text).asOrderedText(), x, y, DESC_COLOR);
    }

    private void drawOrderedText(DrawContext ctx, OrderedText text, int x, int y, int color) {
